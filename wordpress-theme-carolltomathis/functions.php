...
function carolltomathis_scripts() {
    // Enqueue Google Fonts first (before theme stylesheet)
    ...
    wp_enqueue_style('carolltomathis-style', get_stylesheet_uri(), array('google-fonts'), wp_get_theme()->get('Version'));
 ...
 */
function carolltomathis_remove_comment_url_field($fields) {
    if (isset($fields['url'])) {
        unset($fields['url']);
    }
    return $fields;
}
add_filter('comment_form_default_fields', 'carolltomathis_remove_comment_url_field');
