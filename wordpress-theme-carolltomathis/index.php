    ...
    <div class="max-w-4xl mx-auto space-y-12">
        <div class="space-y-4">
            ...
            <h1 class="font-display text-4xl font-bold leading-tight text-champagne" style="font-size: 2.5rem;">
                <?php
                if (is_home()) {
                    _e('Artigos e Reflexões', 'carolltomathis');
                } elseif (is_category()) {
                    single_cat_title();
                } elseif (is_tag()) {
                    single_tag_title();
                    ...
                    printf(__('Resultados da busca por: %s', 'carolltomathis'), '<span>' . get_search_query() . '</span>');
                } else {
                    _e('Artigos e Reflexões', 'carolltomathis');
                }
                ?>
            ...
            <?php if (is_home() || is_category() || is_tag()) : ?>
                <p class="text-base text-champagne" style="color: rgba(249, 229, 216, 0.8);">
                    <?php _e('Conteúdo sobre relacionamentos, recomeços e autoconhecimento.', 'carolltomathis'); ?>
                </p>
            <?php endif; ?>
        </div>
        ...
        <?php else : ?>
            <div class="card-soft text-center py-12">
                <p style="color: rgba(249, 229, 216, 0.7);">
                    <?php _e('Nenhum post encontrado.', 'carolltomathis'); ?>
                </p>
            </div>
        <?php endif; ?>
    </div>
